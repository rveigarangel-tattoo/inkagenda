    ...
    <aside className="hidden lg:flex flex-col w-64 fixed inset-y-0 left-0 bg-[#0d0d0d] border-r border-[#1f1f1f] z-40">
      <div className="p-6 border-b border-[#1f1f1f]">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-xl bg-[#d4a853]/10 border border-[#d4a853]/20 flex items-center justify-center"><Zap className="w-4 h-4 text-[#d4a853]" fill="currentColor" /></div>
          <div><h1 className="font-bold text-white text-lg leading-none">InkAgenda</h1><p className="text-[10px] text-[#444] mt-0.5 uppercase tracking-wider">Studio Management</p></div>
        </div>
      </div>
      ...
      <div className="p-4 border-t border-[#1f1f1f]">
        <div className="flex items-center gap-3 mb-3 px-1">
          ...
          <div className="min-w-0 flex-1"><p className="text-sm font-semibold text-white truncate">{session.user?.name}</p><p className="text-xs text-[#555]">{role === "ADMIN" ? "Administrador" : "Tatuador(a)"}</p></div>
        </div>
        <button onClick={() => signOut({ callbackUrl: "/login" })} className="w-full flex items-center gap-2 px-3 py-2 text-sm text-[#666] hover:text-red-400 hover:bg-red-500/5 rounded-xl transition-all"><LogOut className="w-4 h-4" /> Sair</button>
      </div>
    </aside>
  )
}
