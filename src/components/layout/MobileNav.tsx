    ...
    <nav className="lg:hidden fixed bottom-0 left-0 right-0 bg-[#0d0d0d]/95 backdrop-blur-md border-t border-[#1f1f1f] z-50">
      <div className="flex items-center justify-around px-2 py-2">
        {items.map(item => {
          ...
          return (
            <Link key={item.href} href={item.href} className={cn("flex flex-col items-center gap-1 px-3 py-2 rounded-xl transition-all min-w-[52px]", isActive ? "text-[#d4a853]" : "text-[#444]")}>
