...
function CustomTooltip({ active, payload, label }: any) {
  ...
  return (
    <div className="bg-[#1a1a1a] border border-[#2a2a2a] rounded-xl p-3 shadow-xl">
      ...
      <p className="text-sm font-bold text-[#d4a853]">{formatCurrency(payload[0].value)}</p>
      <p className="text-xs text-[#666]">{payload[0].payload.count} sessões</p>
    </div>
  )
}
...
export default function FinancesPage() {
  ...
  const fetchData = (artistId = "") => {
    setLoading(true)
    fetch(artistId ? `/api/finances?artistId=${artistId}` : "/api/finances").then(r => r.json()).then(d => { setData(d); setLoading(false) })
  }
  ...
  return (
    <div className="space-y-5 max-w-2xl mx-auto lg:max-w-none">
      <div className="flex items-center justify-between">
        ...
        {isAdmin && (
          <div className="flex items-center gap-2">
            ...
            <select value={selectedArtist} onChange={e => { setSelectedArtist(e.target.value); fetchData(e.target.value) }} className="px-3 py-2 bg-[#111] border border-[#1a1a1a] rounded-xl text-white text-xs focus:outline-none focus:border-[#d4a853] transition-all">
      ...
      {loading ? <div className="flex items-center justify-center py-12"><Loader2 className="w-5 h-5 text-[#d4a853] animate-spin" /></div> : (
        <>
          <div className="grid grid-cols-3 gap-3">
            {[
              {label:"Receita Total",value:formatCurrency(data?.totalRevenue||0),icon:DollarSign,color:"text-[#d4a853]",bg:"bg-[#d4a853]/10 border-[#d4a853]/10"},
              {label:"Ticket Médio",value:formatCurrency(data?.avgRevenue||0),icon:TrendingUp,color:"text-green-400",bg:"bg-green-500/10 border-green-500/10"},
              {label:"Concluídas",value:data?.completedCount||0,icon:Award,color:"text-blue-400",bg:"bg-blue-500/10 border-blue-500/10"},
            ].map(stat => (
          ...
          <div className="p-4 sm:p-5 bg-[#111] rounded-2xl border border-[#1a1a1a]">
            ...
            <ResponsiveContainer width="100%" height={160}>
              <BarChart data={data?.monthlyData} margin={{ top:0, right:0, left:-24, bottom:0 }}>
                ...
                <Tooltip content={<CustomTooltip />} cursor={{ fill:"#1a1a1a", radius:6 }} />
          ...
          <div className="bg-[#111] rounded-2xl border border-[#1a1a1a] overflow-hidden">
            ...
            <div className="divide-y divide-[#1a1a1a]">
              {data?.appointments?.length === 0 ? (
                <div className="p-8 text-center text-[#555] text-sm">Nenhuma sessão registrada</div>
              ) : data?.appointments?.slice(0,25).map((apt: any) => (
                <div key={apt.id} className="flex items-center gap-3 px-4 py-3 hover:bg-[#1a1a1a] transition-colors">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-white truncate">{apt.client?.name}</p>
                    <p className="text-xs text-[#555]">{SERVICE_LABELS[apt.service]} · {format(new Date(apt.date),"dd/MM/yy",{locale:ptBR})}{isAdmin && <span className="text-[#d4a853]"> · {apt.artist?.name}</span>}</p>
                  </div>
                  <div className="flex flex-col items-end gap-1 shrink-0">
                    <span className="text-sm font-bold text-[#d4a853]">{formatCurrency(apt.price)}</span>
                    <span className={`text-[10px] px-1.5 py-0.5 rounded-full border ${STATUS_COLORS[apt.status]}`}>{STATUS_LABELS[apt.status]}</span>
                  </div>
                </div>
              ))}
            </div>
