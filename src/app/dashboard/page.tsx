  ...
  return (
    <div className="space-y-6 max-w-2xl mx-auto lg:max-w-none">
      <div className="flex items-center justify-between">
        ...
        <Link href="/dashboard/schedule" className="flex items-center gap-2 px-4 py-2.5 bg-[#d4a853] hover:bg-[#c49840] text-black font-bold rounded-xl text-sm transition-all">
          <Plus className="w-4 h-4" /><span className="hidden sm:inline">Novo Agendamento</span><span className="sm:hidden">Novo</span>
        </Link>
      </div>
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {stats.map(stat => (
          <div key={stat.label} className={`p-4 rounded-2xl bg-[#111] border ${stat.border}`}>
            <div className={`inline-flex p-2 rounded-xl ${stat.iconBg} mb-3`}><stat.icon className={`w-4 h-4 ${stat.color}`} /></div>
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          ...
          <Link href="/dashboard/schedule" className="text-xs text-[#d4a853] hover:text-[#c49840] font-medium">Ver todos →</Link>
        </div>
