  ...
  return (
    <div className="space-y-4 max-w-2xl mx-auto lg:max-w-none">
      <div className="flex items-center justify-between">
        ...
        <button onClick={openAdd} className="flex items-center gap-2 px-4 py-2.5 bg-[#d4a853] hover:bg-[#c49840] text-black font-bold rounded-xl text-sm transition-all shadow-lg shadow-[#d4a853]/10">
          <Plus className="w-4 h-4" /> Novo
        </button>
      </div>
      <div className="relative">
        <Search className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-[#444]" />
        <input placeholder="Buscar por nome, telefone ou email..." value={search} onChange={e => setSearch(e.target.value)} className="w-full pl-10 pr-4 py-3 bg-[#111] border border-[#1a1a1a] rounded-xl text-white text-sm placeholder:text-[#444] focus:outline-none focus:border-[#d4a853] transition-all" />
      </div>
      {loading ? <div className="flex items-center justify-center py-12"><Loader2 className="w-5 h-5 text-[#d4a853] animate-spin" /></div> : filtered.length === 0 ? (
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-end sm:items-center justify-center p-4">
          <div className="w-full max-w-md bg-[#111] rounded-2xl border border-[#2a2a2a] shadow-2xl">
            <div className="flex items-center justify-between p-5 border-b border-[#1f1f1f]">
              <h3 className="font-bold text-white">{editClient ? "Editar Cliente" : "Novo Cliente"}</h3>
            ...
            <form onSubmit={handleSubmit} className="p-5 space-y-3">
              {[{label:"Nome *",key:"name",type:"text",placeholder:"Nome completo",required:true},{label:"Telefone",key:"phone",type:"tel",placeholder:"(11) 99999-9999"},{label:"Email",key:"email",type:"email",placeholder:"cliente@email.com"}].map(field => (
                <div key={field.key}>
                  ...
                  <input type={field.type} required={field.required} placeholder={field.placeholder} value={(form as any)[field.key]} onChange={e => setForm(f => ({ ...f, [field.key]: e.target.value }))} className="w-full px-3 py-2.5 bg-[#1a1a1a] border border-[#2a2a2a] rounded-xl text-white text-sm focus:outline-none focus:border-[#d4a853] transition-all placeholder:text-[#444]" />
                </div>
              ))}
              <div>
                <label className="block text-xs font-medium text-[#888] mb-1.5">Observações</label>
                <textarea placeholder="Alergias, preferências..." value={form.notes} onChange={e => setForm(f => ({ ...f, notes: e.target.value }))} rows={2} className="w-full px-3 py-2.5 bg-[#1a1a1a] border border-[#2a2a2a] rounded-xl text-white text-sm focus:outline-none focus:border-[#d4a853] transition-all resize-none placeholder:text-[#444]" />
              </div>
